...
Para assegurar a operabilidade do simulador em qualquer circunstância, foi implementado um mecanismo de \textit{fallback} (recuperação de falhas). Caso o solver CP-SAT atinja o limite de tempo sem encontrar uma solução viável (especialmente em cenários de alta complexidade como o de 15 ou 30 dias), o sistema não retorna um erro ou uma solução vazia.

Em vez disso, o simulador detecta o estado de interrupção e automaticamente recupera a melhor solução viável gerada pela \textit{Heurística Construtiva} na etapa de inicialização. Essa solução de segurança garante que o usuário sempre tenha uma base de planejamento disponível, marcada nos relatórios como uma solução de \textit{fallback}, preservando a estabilidade da ferramenta mesmo sob estresse computacional extremo.
