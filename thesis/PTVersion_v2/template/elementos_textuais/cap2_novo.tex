...
Adicionalmente, a revisão destaca lacunas existentes na literatura, em especial no que se refere à integração profunda entre conformidade regulatória, granularidade temporal fina, análise estrutural do modelo e experimentação híbrida, aspectos que motivam e justificam a abordagem proposta nesta dissertação. Por fim, este capítulo estabelece o posicionamento científico do trabalho, identificando suas principais contribuições em relação ao estado da arte.
...
Além disso, \cite{taylor2019introduction} destaca que problemas de escalonamento frequentemente são NP-difíceis e exigem formulações que lidem com alta dimensionalidade. Restrições de janelas móveis, comuns em regulamentações como a europeia, exigem especial atenção para evitar inconsistências ou sobrecargas de condução.
...
A Programação Linear Inteira (PLI) é amplamente utilizada para modelar problemas combinatórios na área de logística. \cite{hillier2021introduction} destaca que variáveis binárias e restrições lineares permitem representar condições complexas como limites de tempo, dependências condicionais e estruturas acumulativas – características essenciais no escalonamento sob regulamentação.
...
Há uma diversidade de estudos aplicados ao escalonamento e otimização no contexto de logística. \cite{moreira2024modeling} aborda minimização de custos logísticos em empresas de transporte rodoviário, enquanto \cite{moreira2025optimization} apresenta um modelo preliminar para escalonamento de motoristas sob regulamentação.
...
A análise revela que cada trabalho atua em um eixo metodológico complementar. O estudo de \cite{blochliger2004modeling} introduz a integração fundamental entre pausas e roteirização, conceito que é refinado em trabalhos posteriores sobre VRP. Savelsbergh e Sol (1995) fornecem a estrutura matemática que dá base a praticamente todos os modelos de \textit{pickup-and-delivery} e variantes modernas de VRP.

Os trabalhos de \cite{moreira2024modeling,moreira2025optimization} avançam o estado da arte ao aplicar formulações exatas completas para o problema de escalonamento sob as normas europeias. O trabalho mais recente apresenta o modelo mais completo da literatura no que diz respeito à conformidade temporal com todas as restrições do Regulamento (CE) n.º 561/2006, superando abordagens anteriores tanto em granularidade quanto em completude.

O estudo de \cite{pillai2019compliance}, por sua vez, atua em um espaço intermediário: combina MILP com heurísticas de correção, alcançando bons resultados de conformidade sem necessidade de modelos totalmente exatos em grandes horizontes.
