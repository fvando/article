...
Os elementos pré-textuais apresentados a seguir têm como finalidade orientar o leitor acerca da estrutura gráfica, simbólica e organizacional propostos, dedicada ao desenvolvimento de um modelo de Programação Linear Inteira para o escalonamento de motoristas sob o Regulamento (CE) n.{\textordmasculine} 561/2006.
...
O \textbf{Capítulo 2} é dedicado à revisão da literatura, abordando os fundamentos do transporte rodoviário europeu, as restrições regulatórias aplicáveis, os principais problemas de escalonamento, técnicas de modelagem com Programação Linear Inteira, metaheurísticas, o uso de solvers modernos como o OR-Tools CP-SAT, bem como trabalhos correlatos relevantes.

O \textbf{Capítulo 3} apresenta as bases legais e normativas do escalonamento de motoristas, com uma análise detalhada do Regulamento (CE) n.º 561/2006, da Diretiva 2002/15/CE e do Regulamento (UE) n.º 165/2014, discutindo suas implicações diretas na formulação do modelo matemático.

O \textbf{Capítulo 4} descreve a metodologia adotada na pesquisa, detalhando a estrutura geral do estudo, as etapas de formulação matemática, implementação computacional, definição dos cenários experimentais e os métodos de avaliação das soluções. Este capítulo também introduz a abordagem heurística e matheurística proposta, incluindo a utilização de Large Neighborhood Search (LNS) e a integração opcional com técnicas de aprendizagem de máquina.

O \textbf{Capítulo 5} apresenta o modelo matemático proposto, incluindo a discretização temporal, a definição de conjuntos, parâmetros e variáveis de decisão, as funções objetivo e o conjunto completo de restrições que garantem a conformidade com a legislação europeia, além de uma discussão sobre linearização e observações computacionais.

O \textbf{Capítulo 6} trata da implementação computacional do modelo, descrevendo a arquitetura geral da solução, a interface de parametrização, a construção da matriz de restrições, os modos de resolução do solver, os fluxos computacionais do simulador, bem como os mecanismos de visualização, análise iterativa e geração de indicadores operacionais.

O \textbf{Capítulo 7} apresenta os resultados obtidos a partir dos experimentos realizados, incluindo a configuração dos cenários, a análise do comportamento da demanda, a comparação entre demanda e alocação, os indicadores operacionais, a avaliação dos diferentes modos de resolução e a síntese geral dos resultados.

O \textbf{Capítulo 8} realiza uma revisão comparativa da literatura, confrontando os modelos e técnicas desenvolvidos neste trabalho com o estado da arte, incluindo uma análise crítica das abordagens existentes e a discussão do pipeline híbrido de otimização proposto.

Por fim, o \textbf{Capítulo 9} apresenta as conclusões do trabalho, sintetizando as principais contribuições científicas, os resultados alcançados, as limitações identificadas e as direções para trabalhos futuros.

As referências bibliográficas, o glossário, os apêndices e os anexos complementam o trabalho, fornecendo detalhamentos adicionais sobre formulações alternativas do modelo, cenários de simulação, parâmetros utilizados, estruturas de dados, fluxos da aplicação e documentação normativa.
