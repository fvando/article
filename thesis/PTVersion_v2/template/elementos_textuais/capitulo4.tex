...
\subsection*{Início e fim do turno}
\[
S_{d,t}, E_{d,t} \in \{0,1\}, \quad \text{início e fim de turno do motorista } t \text{ no período } d.
\]

Essas variáveis auxiliares garantem que cada motorista tenha exatamente um
início e um fim de turno, assegurando continuidade temporal da presença.
...
\section{Função Objetivo Lexicográfica}\footnote{A estratégia de otimização
lexicográfica apresentada nesta seção representa a implementação final desenvolvida
durante a pesquisa, incorporando refinamentos metodológicos realizados após a
qualificação do projeto. A abordagem de solves sequenciais independentes demonstrou
desempenho superior em testes comparativos, garantindo separação rigorosa entre
prioridades hierárquicas dos objetivos conflitantes.}

O modelo implementa uma estrutura de otimização lexicográfica verdadeira,
com duas fases resolvidas através de solves consecutivos independentes.
Cada fase possui um orçamento temporal próprio, permitindo convergência
adequada antes de avançar para o próximo critério hierárquico.

\subsection{Fase 1 — Cobertura da Demanda (40\% do tempo total)}

A Fase 1 possui prioridade absoluta e busca minimizar a demanda não atendida:
\[
\min \; \sum_{d \in D} \left(1 + \epsilon \cdot \frac{d}{|D|}\right) U_d
+ \delta \sum_{t \in T} Z_t,
\]
onde $\epsilon = 0.0001$ é um fator de desempate temporal (favorece atendimento
em períodos iniciais) e $\delta = 0.1$ é um peso moderado para incentivo
preliminar de redução de motoristas.

Após convergência da Fase 1, registra-se o valor ótimo $U^* = \sum_{d \in D} U_d$
e procede-se para a Fase 2.

Estruturas lexicográficas com solves sequenciais são amplamente utilizadas
quando múltiplos objetivos conflitantes devem ser tratados de forma estritamente
hierárquica \cite{pillac2013, erdman2022}.

\subsection{Fase 2 — Minimização de Recursos (60\% do tempo total)}

Com a cobertura da demanda garantida pela Fase 1, a Fase 2 fixa o valor
de demanda não atendida através da restrição:
\[
\sum_{d \in D} U_d \le U^* + \varepsilon_{tol},
\]
onde $\varepsilon_{tol} = 0.1$ é uma margem de tolerância numérica para
evitar infactibilidade por erros de arredondamento.

Com essa restrição ativa, a Fase 2 resolve um novo problema de otimização:
\[
\min \; \sum_{t \in T} Z_t - \theta \sum_{d \in D} \sum_{t \in T} X_{d,t},
\]
onde $\theta = 0.001$ é um peso de desempate que favorece maior utilização
da capacidade dos motoristas ativados (critério terciário).

Essa abordagem em duas etapas garante prioridade absoluta da cobertura
da demanda, seguida de minimização rigorosa do número de motoristas,
com critério de desempate baseado em maximização de atribuições.
A separação em solves independentes evita compromissos prematuros entre
objetivos hierarquicamente distintos, permitindo exploração mais efetiva
do espaço de soluções em cada fase \cite{savelsbergh1995, erdman2022}.
...
\subsection{Continuidade do turno}

Cada motorista ativo deve ter exatamente um início e um fim de turno:
\[
\sum_{d \in D} S_{d,t} = Z_t, \quad \sum_{d \in D} E_{d,t} = Z_t, \quad \forall t.
\]

A evolução temporal da presença deve ser consistente com início e fim:
\[
Y_{d,t} - Y_{d-1,t} = S_{d,t} - E_{d,t}, \quad \forall d > 1, \forall t.
\]

Essas restrições garantem turnos contínuos (sem interrupções intradiárias),
fundamentais para modelar de forma realista a jornada de trabalho dos motoristas.

\subsection{Duração mínima e máxima do turno}

O turno de cada motorista ativo deve respeitar limites operacionais:
\[
18 \cdot Z_t \le \sum_{d \in D} Y_{d,t} \le 52 \cdot Z_t, \quad \forall t,
\]
onde 18 períodos (4,5 horas) representa a duração mínima viável para
justificar ativação de um motorista, e 52 períodos (13 horas) é o limite
máximo considerando pausas e conformidade regulatória realista.
...
\subsection{Estratégia de Resolução Lexicográfica}

A implementação computacional utiliza uma abordagem de otimização lexicográfica
verdadeira, com dois solves independentes executados sequencialmente:

\begin{enumerate}
    \item \textbf{Fase 1 (Cobertura)}: Recebe 40\% do orçamento temporal total,
          focando exclusivamente na minimização da demanda não atendida. Ao término,
          registra-se o valor ótimo $U^*$ obtido.
    
    \item \textbf{Fase 2 (Recursos)}: Utiliza os 60\% restantes do tempo, com a
          restrição $\sum U_d \le U^* + 0.1$ ativada. Minimiza o número de
          motoristas ativados, garantindo que a cobertura da Fase 1 seja preservada
          (tolerância numérica de 0.1 slot).
\end{enumerate}

Essa divisão temporal e estrutural evita compromissos prematuros entre objetivos
hierarquicamente distintos, permitindo que cada fase explore adequadamente o
espaço de soluções antes de fixar resultados parciais.

\subsection{Parâmetros de Controle do Solver}

A execução do solver SCIP (via OR-Tools) é configurada com os seguintes parâmetros:

\begin{table}[H]
\centering
\caption{Parâmetros de controle do solver SCIP.}
\label{tab:solver-params}
\begin{tabular}{lll}
\toprule
\textbf{Parâmetro} & \textbf{Valor} & \textbf{Descrição} \\
\midrule
\texttt{limits/time} & 300s (total) & Tempo máximo de execução (5 minutos) \\
\texttt{limits/gap}  & 0.05 (5\%)   & Gap de otimalidade aceitável \\
Fase 1               & 120s (40\%)  & Orçamento temporal para cobertura \\
Fase 2               & 180s (60\%)  & Orçamento temporal para recursos \\
$\varepsilon_{tol}$  & 0.1          & Tolerância numérica entre fases \\
\bottomrule
\end{tabular}
\end{table}

Esses valores foram calibrados empiricamente para equilibrar qualidade da solução
e tempo de resposta compatível com uso operacional. Instâncias de maior porte
podem requerer ajustes proporcionais no limite temporal.
