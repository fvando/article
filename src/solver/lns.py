...
import numpy as np
from src.ml.ml_guidance import neighborhood_scorer
...
def run_lns(
    ...
    radio_selection_object: Any = None,
    ...
    # 0) Record initial state (Heuristic baseline)
    history.append({
        "iteration": -1, # Baseline
        "total_active_drivers": best_total_active_drivers,
        "total_assigned_slots": best_total_assigned_slots,
        "improved": True,
        "status": "INITIAL"
    })
    ...
    for it in range(int(max_lns_iterations)):
        ...
        # ---- unpack defensivo (compatível com solver engine.py) ----
        # Engine: (solver, status, total_active, total_slots, workers_schedule, tasks_schedule, 
        #          driving_hours, constraints, initial_density, final_density, statistics_result, 
        #          msg_list?, iter_data?, matrix_allocation, logs)
        ...
        # result[5] é tasks_schedule
        # result[6] é driving_hours
        # result[7] é constraints_coefficients
        initial_density      = result[8]
        final_density        = result[9]
        statistics_result    = result[10]
        msg                  = result[11] # Engine retorna [] nas posições 11 e 12?? Ver engine.py:650
        iterations_data      = result[12]
        matrix_allocation    = result[13]
        solver_logs          = result[14]
        ...
        # Se o sub-problema retornou tudo zero (incoerente ou time-out sem feitura), tratamos como descarte
        if new_total_assigned_slots == 0:
            history.append({
                "iteration": it,
                "total_active_drivers": best_total_active_drivers,
                "total_assigned_slots": best_total_assigned_slots,
                "improved": False,
                "status": f"ZERO_REJECTED_{status}"
            })
            continue
        
        # Calculate a weighted score for comparison (lower is better)
        # We must prioritize coverage (unmet demand) over driver count
        total_demand = sum(need)
        new_unmet = max(0, total_demand - new_total_assigned_slots)
        best_unmet = max(0, total_demand - best_total_assigned_slots)
        
        # W_UNMET must be significantly higher than W_DRIVER to match engine.py logic
        W_UNMET = 10000.0
        W_DRIVER = 100.0
        
        new_score = (new_unmet * W_UNMET) + (new_total_active_drivers * W_DRIVER)
        best_score = (best_unmet * W_UNMET) + (best_total_active_drivers * W_DRIVER)
        
        improved = (new_score < best_score - 0.01) # Small epsilon for float stability
        
        # Safety: log metrics
        if it % 1 == 0:
            print(f"LNS It {it}: New(C={new_total_assigned_slots}, D={new_total_active_drivers}) vs Best(C={best_total_assigned_slots}, D={best_total_active_drivers}) -> Improved: {improved}")
