...
def train_f1() -> Dict[str, Any]:
    ...
    if lgb is None or LGBMClassifier is None:
        return _empty_f1_result("LightGBM não está instalado no ambiente.")

    if not os.path.exists(F1_DATASET_PATH):
        return _empty_f1_result(
            f"Dataset f₁ não encontrado em: {F1_DATASET_PATH}.\n"
            f"Gere o dataset primeiro (ex.: via builder) e tente novamente."
        )

    try:
        df = pd.read_csv(F1_DATASET_PATH)
    except Exception as e:
        return _empty_f1_result(f"Erro ao ler dataset f₁: {e}")

    if df.empty:
        return _empty_f1_result("Dataset f₁ está vazio. Nenhuma linha para treinar.")

    if "label" not in df.columns:
        return _empty_f1_result("Dataset f₁ não contém coluna 'label'.")
    ...
    if not feature_cols:
        return _empty_f1_result(
            "Dataset f₁ não possui colunas de features compatíveis "
            f"(esperadas: {candidate_features})."
        )
    ...
    if n < 20:
        ...
        msg_prefix = f"Dataset f₁ tem poucas linhas ({n}). Modelo pode ficar fraco."
    else:
        msg_prefix = f"Dataset f₁ carregado ({n} linhas)."
    ...
    try:
        ...
        return _empty_f1_result(f"Erro ao treinar LightGBM f₁: {e}")
    ...
    try:
        ...
        return _empty_f1_result(f"Erro ao calcular acurácia de f₁: {e}")
    ...
    try:
        ...
        return _empty_f1_result(f"Erro ao salvar modelo f₁ em {F1_MODEL_PATH}: {e}")

    return {
        "success": True,
        "message": msg_prefix + " Treino concluído com sucesso.",
        "model_path": F1_MODEL_PATH,
        "rows_used": int(n),
        "accuracy": accuracy,
    }
...
def train_f2() -> Dict[str, Any]:
    ...
    if lgb is None or LGBMRegressor is None:
        return _empty_f2_result("LightGBM não está instalado no ambiente.")

    if not os.path.exists(F2_DATASET_PATH):
        return _empty_f2_result(
            f"Dataset f₂ não encontrado em: {F2_DATASET_PATH}.\n"
            f"Gere o dataset primeiro (ex.: via builder) e tente novamente."
        )
    ...
    try:
        ...
        return _empty_f2_result(f"Erro ao ler dataset f₂: {e}")

    if df.empty:
        return _empty_f2_result("Dataset f₂ está vazio. Nenhuma linha para treinar.")
    ...
    if target_col is None:
        return _empty_f2_result(
            "Dataset f₂ não contém coluna 'label' nem 'improvement'."
        )
    ...
    try:
        ...
        return _empty_f2_result(f"Erro ao treinar LightGBM f₂: {e}")
    ...
    try:
        ...
        return _empty_f2_result(f"Erro ao calcular MSE de f₂: {e}")
    ...
    try:
        ...
        return _empty_f2_result(f"Erro ao salvar modelo f₂ em {F2_MODEL_PATH}: {e}")
