...
objective_details = {
    "Maximize Demand Response": [
        {
            "Term": "Total served demand",
            "Notation": r"$\sum_{d \in D} \sum_{t \in T} X_{d,t}$",
            "Role": "Maximization",
            "Explanation": "Maximizes the total number of tasks served across all periods."
        }
    ],

    "Minimize Total Number of Drivers": [
        {
            "Term": "Unmet demand penalty",
            "Notation": r"$\alpha \sum_{d \in D} U_d$",
            "Role": "Primary (Phase 1)",
            "Explanation": "Absolute priority: penalizes unmet demand."
        },
        {
            "Term": "Active drivers penalty",
            "Notation": r"$\beta \sum_{t \in T} Z_t$",
            "Role": "Secondary (Phase 1)",
            "Explanation": "Minimizes the number of active drivers."
        },
        {
            "Term": "Task reward",
            "Notation": r"$-\gamma \sum_{d \in D} \sum_{t \in T} X_{d,t}$",
            "Role": "Tie-breaker",
            "Explanation": "Encourages higher utilization of active drivers."
        },
        {
            "Term": "Presence minimization",
            "Notation": r"$\omega_1 \sum_{d,t} Y_{d,t}$",
            "Role": "Phase 2",
            "Explanation": "Reduces unnecessary presence time."
        },
        {
            "Term": "Overload balancing",
            "Notation": r"$\omega_2 \sum_t \max(0, L_t - L^{soft})$",
            "Role": "Phase 2",
            "Explanation": "Balances excessive workload per driver."
        },
        {
            "Term": "Maximum load smoothing",
            "Notation": r"$\omega_3 L_{\max}$",
            "Role": "Phase 2",
            "Explanation": "Avoids load concentration on a single driver."
        }
    ]
}
...
def render_math_table(rows):
    table_md = """
| Term | Notation | Role | Explanation |
|------|----------|------|-------------|
"""
    for r in rows:
        table_md += (
...
lex_objectives = {
    "Minimize Total Number of Drivers": [
        {
            "priority": 1,
            "name": "Unmet demand minimization",
            "formula": r"\min \sum_{d \in D} U_d",
            "description": "Absolute priority: maximize coverage."
        },
        {
            "priority": 2,
            "name": "Driver minimization",
            "formula": r"\min \sum_{t \in T} Z_t",
            "description": "Minimize the number of active drivers."
        },
        {
            "priority": 3,
            "name": "Utilization tie-breaker",
            "formula": r"\max \sum_{d \in D} \sum_{t \in T} X_{d,t}",
            "description": "Avoid idle activated drivers."
        },
        {
            "priority": 4,
            "name": "Workload balancing",
            "formula": r"\min \left(\sum Y_{d,t} + \sum \max(0, L_t - L^{soft}) + L_{\max}\right)",
            "description": "Applied after fixing optimal U and Z."
        }
    ]
}
...
def plot_demand_vs_capacity_2(df):
    ...
    ax.set_ylabel("Drivers")
    ax.set_title("Demand vs. Assigned Capacity")
    ax.grid(axis="y", linestyle=":", alpha=0.6)
    ax.legend()
...
def plot_capacity_gap(df):
    ...
    ax.set_xlabel("Time Slot")
    ax.set_ylabel("Gap (Capacity − Demand)")
    ax.set_title("Capacity Gap per Slot")
    ax.grid(axis="y", linestyle=":", alpha=0.6)
    ...
    ax.annotate(
        f"Max Deficit = {worst['Motoristas'] - worst['Demanda']}",
...
with st.expander("Objective", expanded=True):
        ...
        with st.expander("Detail", expanded=False):
            ...
            st.markdown("### Lexicographic Objective Structure")
...
with st.expander("Restrinctions", expanded=True):
    selected_restrictions = {}
...
with st.expander("Paremeters", expanded=True):
        ...
        if len(need) != num_periods:
            ...
            with col_resultsI_col:
                ...
                with st.expander("Interpreter", expanded=False):                             
                    ...
                    st.markdown("## 🧠 Interpretation of the Result")
                    st.success(interp["model_state"])

                    st.markdown("### 📊 Operational Efficiency")
                    st.json(interp["operational_metrics"])

                    st.markdown("### ⚙️ Model Structure")
                    ...
                    st.subheader("⚙️ Solver Behavior")
                ...
                # Esforço Operacional
                #--------------------------                         
                with st.expander("Operational Effort", expanded=True):     
