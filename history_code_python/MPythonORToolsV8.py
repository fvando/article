...
def relax_objective(solver, Y, num_periods, limit_Workers, penalty, max_penalty, relaxation_level):
    """
...
def greedy_allocation(demanda, max_motoristas, num_periods, limit_Workers, max_demand_per_driver):
    """
    Alocação inicial de motoristas utilizando uma abordagem gulosa, onde um motorista pode ser alocado 
    para mais de uma demanda, conforme a capacidade de atendimento.

    :param demanda: Lista de demandas por slot (por exemplo, de 96 períodos de 15 minutos).
    :param max_motoristas: Número máximo de motoristas disponíveis para alocação.
    :param num_periods: Número de períodos totais (por exemplo, 96 períodos de 15 minutos por dia).
    :param limit_Workers: Número máximo de motoristas alocados por período.
    :param max_demand_per_driver: Número máximo de demandas que um motorista pode atender durante o dia.
    :return: Matriz de alocação inicial de motoristas (Y[d, t]).
    """
    allocation = np.zeros((num_periods, limit_Workers), dtype=int)  # Inicializa a alocação com 0 motoristas
    
    # Exibe a alocação (para fins de visualização)
    # print(allocation)
    
    # Ordena os períodos pela demanda (do maior para o menor)
    sorted_periods = np.argsort(demanda)[::-1]
    
    # Cria uma lista para contar quantas demandas cada motorista atendeu
    drivers_demand_count = np.zeros(limit_Workers, dtype=int)
    
    # Atribui motoristas para os períodos de maior demanda
    for period in sorted_periods:
        needed_motorists = demanda[period]
        
        # Aloca motoristas, respeitando o número máximo de motoristas por período
        for t in range(limit_Workers):
            if needed_motorists > 0 and drivers_demand_count[t] < max_demand_per_driver:
                allocation[period, t] = 1  # Aloca um motorista no período (Y[d, t] = 1)
                drivers_demand_count[t] += 1  # Incrementa a contagem de demandas atendidas pelo motorista
                needed_motorists -= 1  # Reduz a demanda restante
        
        # Se a demanda para o período ainda não for atendida, tentar redistribuir motoristas
        while needed_motorists > 0:
            for t in range(limit_Workers):
                if drivers_demand_count[t] < max_demand_per_driver:
                    allocation[period, t] = 1
                    drivers_demand_count[t] += 1
                    needed_motorists -= 1
                    if needed_motorists == 0:
                        break
    # Exibe a alocação (para fins de visualização)
    # print(allocation)
...
def solve_shift_schedule(solverParamType, need, variable_type, constraints_coefficients, selected_restrictions, swap_rows=None, multiply_row=None, add_multiple_rows=None, densidadeAceitavel=None, limit_Workers=0,limit_Iteration=0,limit_Level_Relaxation=0, max_demands_per_driver=1,tolerance_Demands=0.01, penalty = 0.01):
...
# Descanso Após Trabalho	Requer descanso após 6 dias consecutivos de trabalho.	Complementa os limites semanais e quinzenais, mas pode criar conflitos em semanas críticas.
    ...
    if finalDensity <= densidadeAceitavel:
        ...
        while status != pywraplp.Solver.OPTIMAL and iteration < max_iterations:
            ...
            objective = relax_objective(solver, Y, num_periods, limit_Workers, penalty, max_penalty, relaxation_level)
...
with st.form("paramModel"):
    with st.expander("", expanded=True):    
        ...
        with col4:
            ...
            limit_Workers = st.number_input("Drivers (0, no limit)", min_value=100,  key="limit_workers") #int(math.ceil(sum(needNew) / int(max_demands_per_driver))),  key="limit_workers")
...
try:
    ...
    if len(need) != num_periods:
                ...
                if (not swap_rows_c 
                    ...
                    if swap_rows_c:
                        with col_resultsItIOpI:
                            ...
                            swap_rows = row1, row2
                            constraints_coefficients = load_data('constraints_coefficients.json')
                            solver, status, total_workers, workers_schedule, constraints_coefficients, initialDensity, finalDensity, statisticsResult, msgResult, iterations_dataResult, matrixAllocation = solve_shift_schedule(solverParamType, need, variable_type, constraints_coefficients, selected_restrictions, swap_rows=swap_rows, multiply_row=None, add_multiple_rows=None,densidadeAceitavel=acceptable_percentage,limit_Workers=limit_Workers,limit_Iteration=limit_Iteration,limit_Level_Relaxation=limit_Level_Relaxation, max_demands_per_driver=max_demands_per_driver,tolerance_Demands=tolerance_Demands, penalty=penalty)
                    if multiply_row_c:
                        with col_resultsItIOpII:
                            ...
                            if constant != 0:
                                ...
                                constraints_coefficients = load_data('constraints_coefficients.json')
                                solver, status, total_workers, workers_schedule, constraints_coefficients, initialDensity, finalDensity, statisticsResult, msgResult, iterations_dataResult, matrixAllocation = solve_shift_schedule(solverParamType, need, variable_type, constraints_coefficients, selected_restrictions, swap_rows=None,multiply_row=multiply_row, add_multiple_rows=None,densidadeAceitavel=acceptable_percentage,limit_Workers=limit_Workers,limit_Iteration=limit_Iteration,limit_Level_Relaxation=limit_Level_Relaxation, max_demands_per_driver=max_demands_per_driver,tolerance_Demands=tolerance_Demands, penalty=penalty)
                            else:
                    ...
                    if add_multiple_rows_c:
                        with col_resultsItIOpIII:
                            ...
                            solver, status, total_workers, workers_schedule, constraints_coefficients, initialDensity, finalDensity, statisticsResult, msgResult, iterations_dataResult, matrixAllocation = solve_shift_schedule(solverParamType, need, variable_type, constraints_coefficients, selected_restrictions, swap_rows=None, multiply_row=None, add_multiple_rows=add_multiple_rows,densidadeAceitavel=acceptable_percentage,limit_Workers=limit_Workers,limit_Iteration=limit_Iteration,limit_Level_Relaxation=limit_Level_Relaxation, max_demands_per_driver=max_demands_per_driver,tolerance_Demands=tolerance_Demands, penalty=penalty)
                        ...
                        if total_workers is not None:
                            ...
                            with col_resultsybdetail_I:
                                    ...
                                    with st.expander("Análise", expanded=True):
                                        # Exibir as métricas
                                        st.write(f"**Desvio Padrão Taxa Cobertura:** {desvio_padrao:.2f}")
                                        ...
                                        st.markdown(f"<h3 style='color:{desvio_cor}; font-weight: bold; font-size: 16px;'>Desvio Padrão da Taxa de Cobertura: {desvio_padrao:.2f} ({status})</h3>", unsafe_allow_html=True)
        ...
        #     if msgResult is None:
        #         #Exibir a densidade
        #         st.subheader("Constraints Matrix Heuristic")
        #         # st.write(f"Final Density Matrix Heuristic: {finalDensity:.4f}")
        #         with st.expander("MatrizHeuristic", expanded=True):
        #             figNew, axNew = plt.subplots(figsize=(14, 8))
        #             # constraints_coefficients = load_data('constraints_coefficients.json')
                    
        #             sns.heatmap(matrixAllocation, cmap="Oranges", cbar=False, annot=False, fmt="d", annot_kws={"size": 6})
        #             plt.title('Constraints Matrix Heuristic')
        #             plt.xlabel('X')
        #             plt.ylabel('Period')
        #             st.pyplot(figNew)
                    
        #             # Calculando o número total de motoristas alocados por período
        #             motoristas_por_periodo = matrixAllocation.sum(axis=1)
                    
        #             # Usando st.bar_chart para exibir o gráfico de barras
        #             st.bar_chart(motoristas_por_periodo)
                                    
                    # figNew, axNew = plt.subplots(figsize=(14, 8))
    
                    # # Gráfico de barras para mostrar o número de motoristas alocados por período
                    # axNew.bar(range(num_periods), motoristas_por_periodo, color="orange")
                    
                    # # Personalizar o gráfico
                    # axNew.set_title('Alocação de Motoristas por Período (Heurística)')
                    # axNew.set_xlabel('Períodos de 15 Minutos')
                    # axNew.set_ylabel('Número de Motoristas Alocados')
