...
# Descanso Após Trabalho	Requer descanso após 6 dias consecutivos de trabalho.	Complementa os limites semanais e quinzenais, mas pode criar conflitos em semanas críticas.
    ...
    if finalDensity <= densidadeAceitavel:
        ...
        while status != pywraplp.Solver.OPTIMAL and iteration < max_iterations:
            ...
            objective = relax_objective(solver, Y, num_periods, limit_Workers, penalty, max_penalty, relaxation_level, needNew, tolerance_Demands, max_demands_per_driver)
...
with st.expander("Parameters", expanded=True):
    ...
    for elementalOperation in elementalOperations:
        ...
        selected_operations[elementalOperation["Key"]] = st.checkbox(checkbox_label, key=elementalOperation["Key"], 
                                                                        value=False, disabled=atualizarFicheiro,
                ...
                if (not swap_rows_c 
                    ...
                    if add_multiple_rows_c_auto:
                        ...
                        for idx, row in enumerate(constraints_coefficients):
                            ...
                            # st.write(f"Test Initial density {calculate_density(initualconstraintsCoefficients)}")
