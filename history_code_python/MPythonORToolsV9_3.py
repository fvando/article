...
import html
# from ml.ml_guidance import assignment_scorer
...
def solve_shift_schedule(
    ...
    return solver, status, total_workers, workers_schedule, constraints_coefficients, initial_density, final_density, statistics_result, msg, iterations_data, matrix_allocation, solver_logs
...
with st.form("paramModel"):
    ...
    with st.expander("Machine Learning – Treinamento", expanded=False):

        st.markdown("### Treinar modelos LightGBM (f₁ e f₂)")
        st.caption("""
        - f₁ → modelo de escolha motorista–período\n
        - f₂ → modelo de avaliação de vizinhanças para LNS\n
        """)
        
        submit_button_train = st.form_submit_button("Treinar modelos ML (f₁ & f₂)")  

        if submit_button_train:

            st.info("Iniciando treinamento dos modelos... isso pode levar alguns minutos.")
            ...
            try:
                ...
                log.write("Treinando modelo f₁ e f₂...")

                results = train_all_models()

                progress.progress(1.0)
                log.write("Treinamento concluído!")

                st.success("Processo de treinamento finalizado!")

                # ==========================================================
                # 🔹 RESULTADOS F1
                # ==========================================================
                f1 = results["f1"]
                st.subheader("📌 Modelo f₁ (Assignment)")

                if f1.get("success"):
                    st.success("Modelo f₁ treinado com sucesso!")

                    # Acurácia
                    if "accuracy" in f1 and f1["accuracy"] is not None:
                        st.write(f"**Acurácia:** {f1['accuracy']:.4f}")
                    else:
                        st.info("Acurácia não disponível (dataset pequeno ou métrica não calculada).")

                    # Linhas utilizadas
                    if "rows_used" in f1:
                        st.write(f"**Linhas utilizadas:** {f1['rows_used']}")
                    else:
                        st.write("**Linhas utilizadas:** informação não disponível.")

                    # Caminho do modelo
                    if f1.get("model_path"):
                        st.write(f"**Modelo salvo em:** `{f1['model_path']}`")
                else:
                    st.warning("Modelo f₁ não foi treinado.")
                    st.write(f1.get("message", "Motivo não informado."))

                # ==========================================================
                # 🔹 RESULTADOS F2
                # ==========================================================
                f2 = results["f2"]
                st.subheader("📌 Modelo f₂ (Neighborhood Ranking)")

                if f2.get("success"):
                    st.success("Modelo f₂ treinado com sucesso!")

                    # MSE
                    if "mse" in f2 and f2["mse"] is not None:
                        st.write(f"**MSE:** {f2['mse']:.4f}")
                    else:
                        st.info("MSE não disponível (dataset pequeno ou métrica não calculada).")

                    # Linhas utilizadas
                    if "rows_used" in f2:
                        st.write(f"**Linhas utilizadas:** {f2['rows_used']}")
                    else:
                        st.write("**Linhas utilizadas:** informação não disponível.")

                    # Caminho do modelo
                    if f2.get("model_path"):
                        st.write(f"**Modelo salvo em:** `{f2['model_path']}`")
                else:
                    st.warning("Modelo f₂ não foi treinado.")
                    st.write(f2.get("message", "Motivo não informado."))
    ...
    with st.expander("Machine Learning – Dataset Builder", expanded=False):
        ...
        st.markdown("### Gerar datasets de treinamento para f₁ (assignment) e f₂ (neighborhood)")
        ...
        st.caption(
            "Obs.: instâncias pequenas são recomendadas para resolver o MILP em modo exato "
            "(por exemplo, 24 períodos e 10 motoristas)."
        )

        submit_button_dataset = st.form_submit_button("Gerar Dataset ML (f₁ & f₂)", type="primary")  
        if submit_button_dataset: #st.button("Gerar Dataset ML (f₁ & f₂)"):
            ...
            for inst in range(int(num_instances)):
                ...
                # ----------------------------------------------
                # 3) Solução exata (MILP) via seu pipeline
                #    Usa o modo "Exact" do run_solver_with_mode
                # ----------------------------------------------
                # try:
                #     (
                #         solver,
                ...
                #         constraints_coefficients_opt,
                #         initial_density_opt,
                #         final_density_opt,
                #         statistics_result_opt,
                #         msg_result_opt,
                #         iterations_data_result_opt,
                #         matrix_allocation_opt,
                #         solver_logs_opt,
                #     ) = run_solver_with_mode(
                #         mode="Exact",
                #         need=need_ml,
                #         variable_type=variable_type,               # usa o que já está selecionado na UI
                #         constraints_coefficients=constraints_coefficients,
                #         selected_restrictions=selected_restrictions,
                #         solver_param_type=solver_param_type,
                ...
                #     )
                # except Exception as e:
                #     st.warning(f"Instância {inst}: erro ao chamar o solver exato: {e}")
                #     continue

                # if matrix_allocation_opt is None:
                #     st.warning(f"Instância {inst}: solver não retornou solução (status={status}). Ignorada.")
                #     continue

                # --------------------------------------------------
                # 3) Resolver (Exact como referência)
                # --------------------------------------------------                
                try:
                    (
                        solver,
                ...
                # total_workers_optimal = int(matrix_allocation_opt.sum())

                # ----------------------------------------------
                # 4) Construir linhas f₁ e f₂ desta instância
                # ----------------------------------------------
                # f1_rows = build_f1_rows(
                #     instance_id=inst,
                #     need=need_ml,
                #     allocation_optimal=matrix_allocation_opt,
                # )
                ...
                #     need=need_ml,
                #     allocation_optimal=matrix_allocation_opt,
                #     total_workers_greedy=total_workers_greedy,
                #     total_workers_optimal=total_workers_optimal,
                #     neighborhood_id=0,
                #     periods=None,  # todos os períodos
                # )

                # f1_rows_all.extend(f1_rows)
                # f2_rows_all.extend(f2_rows)

                # # Pequena pausa para UI ficar responsiva
                # time.sleep(0.05)
                ...
                # st.success(
                #     f"Datasets gerados com sucesso ✅\n\n"
                #     f"- f₁: {len(f1_rows_all)} linhas → {f1_path}\n"
                #     f"- f₂: {len(f2_rows_all)} linhas → {f2_path}"
                # )
            ...
            # # ----------------------------------------------
            # # 5) Salvar datasets nos CSVs
            # # ----------------------------------------------
            # f1_path, f2_path = save_datasets(f1_rows_all, f2_rows_all)
            ...
            # if f1_rows_all:
            #     st.write("Pré-visualização f₁:")
            #     st.dataframe(pd.DataFrame(f1_rows_all).head())
            # if f2_rows_all:
            #     st.write("Pré-visualização f₂:")
            #     st.dataframe(pd.DataFrame(f2_rows_all).head())
    ...
    try:
        ...
        # st.write("DEBUG matrix_allocation:")
        # st.write(type(matrix_allocation))
        # st.write(
        #     None if matrix_allocation is None else matrix_allocation.shape
        # )
        
        # kpis, coverage, safety_margin = compute_kpis(demanda, workers_schedule)
        ...
        with colA:
            ...
            #     st.metric("Global Coverage Score", f"{coverage:.3f}")
            # else:
        ...
        # with colA:
        #     # st.metric("Global Coverage Score", f"{kpis['global_coverage_score']:.3f}")
        #     if coverage is not None and not np.isnan(coverage):
        #         st.metric("Global Coverage Score", f"{coverage:.3f}")
        #     else:
        #         st.metric("Global Coverage Score", "N/A")
            
        #     # st.metric("Worker Efficiency", f"{kpis['worker_efficiency']:.3f}")
        #     we = kpis.get("worker_efficiency")

        #     if we is None:
        #         st.metric("Worker Efficiency", "N/A")
        #     else:
        #         st.metric("Worker Efficiency", f"{we:.3f}")

        # with colB:
        #     st.metric("Operational Risk (%)", f"{kpis['operational_risk']*100:.1f}%")
        #     st.metric("Risk Severity", int(kpis["operational_risk_severity"]))

        # with colC:
        #     st.metric("Estimated Cost (€)", f"{kpis['cost_index']:.2f}")
        #     st.metric("Temporal Stability", f"{kpis['temporal_stability']:.3f}")
        ...
        with colA1:
            # heat1 = plot_heatmap_coverage(coverage)
            # st.pyplot(heat1)
            
            # if isinstance(coverage, (list, np.ndarray)) and len(coverage) > 0:
            #     heat1 = plot_heatmap_coverage(coverage)
            #     st.pyplot(heat1)
            # else:
        ...
        with colB1:
            ...
            # heat2 = plot_heatmap_safety(safety_margin)
            # st.pyplot(heat2)
            
            # if isinstance(safety_margin, (list, np.ndarray)) and len(safety_margin) > 0:
            #     heat2 = plot_heatmap_safety(safety_margin)
            #     st.pyplot(heat2)
            # else:
        ...
        with colA2:
            ...
            #         "GlobalCoverage": kpis["global_coverage_score"],
            #         "Efficiency": kpis["worker_efficiency"],
            #         "Stability": kpis["temporal_stability"],
            #         "Risk": 1 - kpis["operational_risk"],  # invertido
            #     })
            #     st.pyplot(radar)
