...
def solve_shift_scheduleOld(
    ...
    # st.write("selected_restrictions =", selected_restrictions)
    # st.write("pausa_45_minutos flag =", selected_restrictions.get("pausa_45_minutos", False))
    ...
    with contextlib.redirect_stdout(stdout_buffer):
        with contextlib.redirect_stderr(stderr_buffer):    
            ...
            # ==============================
            TIME_LIMIT_SEC = 120        # ou vindo do Streamlit
            GAP_REL = 0.05              # 5%
            ...
            #--------------------------------
            # Blanceamento de cargas
            #-------------------------------               
                                        
            # -------------------------------
            #  (A) Carga por motorista (L[t])
            # -------------------------------
            # L = {}
            # for t in range(limit_workers):
            ...
            # L = {}
            # for t in range(limit_workers):
            #     L[t] = solver.NumVar(0, num_periods, f"L[{t}]")
            #     solver.Add(L[t] == solver.Sum(Y[d, t] for d in range(num_periods)))
            #     solver.Add(L[t] <= num_periods * Z[t])           
           
            # # --- PATCH A1: variável para máxima carga ---
            # # Lmax = solver.NumVar(0, cap_tasks_per_driver_per_slot * num_periods, "Lmax")

            # # Maior carga entre motoristas ativos
            # Lmax = solver.NumVar(0, num_periods, "Lmax")
            ...
            # Dev = {}
            # for t in range(limit_workers):
            #     Dev[t] = solver.NumVar(0, solver.infinity(), f"Dev[{t}]")
                # solver.Add(Dev[t] >= L[t] - L_avg)
                # solver.Add(Dev[t] >= L_avg - L[t])                
                
            # carga média endógena
            # L_avg = solver.NumVar(0, cap_tasks_per_driver_per_slot * num_periods, "L_avg")

            # solver.Add(
            #     L_avg * solver.Sum(Z[t] for t in range(limit_workers))
            #     == solver.Sum(L[t] for t in range(limit_workers))
            # )
                          

            # -------------------------------
            #  (B) Soft cap (balanceamento)
            # -------------------------------
            # SOFT_CAP = 32 * cap_tasks_per_driver_per_slot  # ex.: 8h = 32 slots (15min)
            # Over = {}
            # for t in range(limit_workers):
            #     Over[t] = solver.NumVar(0, cap_tasks_per_driver_per_slot * num_periods, f"Over[{t}]")  # use NumVar
            ...
            #     solver.Add(Over[t] <= cap_tasks_per_driver_per_slot * num_periods * Z[t])
                ...
                # Over[t] = solver.IntVar(0, solver.infinity(), f"Over[{t}]")
                # solver.Add(Over[t] >= L[t] - SOFT_CAP)
                # solver.Add(Over[t] >= 0)
                # solver.Add(Over[t] <= num_periods * Z[t])  # inativo => Over=0
            ...
            # Vínculo de capacidade X ≤ cap · Y
            # for d in range(num_periods):
            #     for t in range(limit_workers):
            #         solver.Add(X[d, t] <= cap_tasks_per_driver_per_slot * Y[d, t])

            # # 🔴 PATCH 1 — produção mínima se o motorista está presente
            # # Evita motorista "fantasma" (Y=1, X=0)
            # for d in range(num_periods):
            #     for t in range(limit_workers):
            #         solver.Add(X[d, t] >= Y[d, t])

            # (Opcional) Se quiser manter X por compatibilidade visual, não deixe X dirigir o modelo.
            # Se mantiver X, prenda X ao Y de modo neutro:
            # for d in range(num_periods):
            #     for t in range(limit_workers):
            #         solver.Add(X[d, t] == Y[d, t])
            ...
            # 3) Ativação correta
            for t in range(limit_workers):
                solver.Add(solver.Sum(Y[d, t] for d in range(num_periods)) >= Z[t])
            ...
            for t in range(limit_workers):
                L[t] = solver.NumVar(0, cap_tasks_per_driver_per_slot * num_periods, f"L[{t}]")
                # carga em "tarefas": some X; se você quer carga em "slots", some Y (veja observação abaixo)
                # solver.Add(L[t] == solver.Sum(X[d, t] for d in range(num_periods)))
                # solver.Add(L[t] <= cap_tasks_per_driver_per_slot * num_periods * Z[t])
                solver.Add(L[t] == solver.Sum(Y[d, t] for d in range(num_periods)))
            ...
            if (radio_selection_object == "Maximize Demand Response"):
                ...
                solver.Maximize(
                    ...
                    - penalty * solver.Sum(
                        # (need[d] * tolerance_demands - solver.Sum(Y[d, t] * max_demands_per_driver for t in range(limit_workers)))
                        # (need[d] - solver.Sum(U[d, t] for t in range(limit_workers)))
            ...
            #     alpha_drivers = (num_periods * limit_workers) + 1 #1000
            #     beta_work = 1
            #     gamma_unmet = penalty

            #     solver.Minimize(
            #         alpha_drivers * solver.Sum(Z[t] for t in range(limit_workers))
            #         + beta_work * solver.Sum(Y[d, t] for d in range(num_periods) for t in range(limit_workers))
            #         + gamma_unmet * solver.Sum(U[d] for d in range(num_periods))
            #     )                
            # elif radio_selection_object == "Minimize Total Number of Drivers":

            #     BIG_UNMET = 1_000_000   # prioridade absoluta: evitar déficit
            #     BIG_Z     = 1_000       # depois: menos motoristas ativos
            #     SMALL_Y   = 1           # depois: menos esforço

            #     solver.Minimize(
            #         BIG_UNMET * solver.Sum(U[d] for d in range(num_periods))
            #         + BIG_Z   * solver.Sum(Z[t] for t in range(limit_workers))
            #         + SMALL_Y * solver.Sum(Y[d, t] for d in range(num_periods) for t in range(limit_workers))
            #     )
                
            # elif radio_selection_object == "Minimize Total Number of Drivers":

            #     # BIG_UNMET = 1_000_000   # prioridade absoluta: não deixar demanda descoberta
            #     # BIG_Z     = 1_000       # depois: menos motoristas ativos
            #     # SMALL_Y   = 1           # depois: menos esforço total
            #     # SMALL_BAL = 5           # por último: balanceamento

            #     # solver.Minimize(
            #     #     BIG_UNMET * solver.Sum(U[d] for d in range(num_periods))
            #     #     + BIG_Z   * solver.Sum(Z[t] for t in range(limit_workers))
            #     #     + SMALL_Y * solver.Sum(Y[d, t] for d in range(num_periods) for t in range(limit_workers))
            #     #     + SMALL_BAL * solver.Sum(Over[t] for t in range(limit_workers))
            #     # )
                
            #     BIG_UNMET = 1_000_000   # tarefas não atendidas
            #     BIG_Z     = 10_000       # motoristas ativos
            #     # REWARD_X   = 1           # trabalho real (tarefas)
            #     SMALL_BAL = 10          # balanceamento
            #     SMALL_X = 1
            #     MID_BAL = 50   # ajuste fino
            ...
            #     # # + SMALL_X * solver.Sum(Over[t] for t in range(limit_workers))
            #     # + SMALL_X * solver.Sum(X[d, t] for d in range(num_periods) for t in range(limit_workers))
            #     # )
                
            #     # solver.Minimize(
            #     #     BIG_UNMET * solver.Sum(U[d] for d in range(num_periods)) # prioridade absoluta
            #     # + BIG_Z     * solver.Sum(Z[t] for t in range(limit_workers)) # menos motoristas
            #     # + SMALL_BAL * solver.Sum(Over[t] for t in range(limit_workers))  # equilíbrio
            #     # )
                
            #     BIG_LMAX  = 100        # pode ajustar
            ...
            if selected_restrictions["cobertura_necessidade"]: 
                # for d in range(num_periods):
                #     # constraint_expr = solver.Sum(Y[d, t] * max_demands_per_driver for t in range(limit_workers)) + U[d] >= tolerance_demands * need[d]
                #     # constraint_expr = solver.Sum(X[d, t] for t in range(limit_workers)) + U[d] >= tolerance_demands  * need[d]
                #     constraint_expr = solver.Sum(X[d, t] for t in range(limit_workers)) + U[d] == need[d]
                ...
                # -------------------------------
                # MODELAGEM CANÔNICA DA DEMANDA
                # -------------------------------
                # for d in range(num_periods):
                #     # não pode atender mais do que existe
                #     solver.Add(solver.Sum(X[d, t] for t in range(limit_workers)) <= need[d])
                #     # variável de falta (slack)
                #     solver.Add(U[d] >= need[d] - solver.Sum(X[d, t] for t in range(limit_workers)))
                #     solver.Add(U[d] >= 0)
                
                # -------------------------------
                # DEMANDA CANÔNICA POR SLOT
                #   sum_t Y[d,t] + U[d] = need[d]
                # -------------------------------
                # for d in range(num_periods):
                #     solver.Add(
                #         solver.Sum(Y[d, t] for t in range(limit_workers)) + U[d] == need[d]
                #     )                
            ...
            if selected_restrictions.get("pausa_45_minutos", False):
                ...
                for t in range(limit_workers):
                    for start in range(num_periods - window + 1):
                        constraints.append(
                            solver.Add(
                                solver.Sum(Y[start + p, t] for p in range(window))
                                <= max_continuous_work
                            )
                        )
            ...
            # if selected_restrictions.get("pausa_45_minutos", False): #pausa_4h30_15min
            #     max_continuous_work = 18  # 4h30 = 18 períodos
            #     min_break = 1             # 15 min
            #     window = max_continuous_work + min_break  # 19
            ...
            if selected_restrictions["limite_diario"]: 
                ...
                for day in range(num_periods // periods_per_day):  # 96 períodos de 15 minutos em um dia
                    ...
                    for t in range(limit_workers):
                        constraint_expr = solver.Sum(Y[d, t] for d in range(day_start, day_end))                    
            ...
            #     for day in range(num_periods // periods_per_day):  # Para cada dia com 96 períodos de 15 minutos
            ...
            #             solver.Sum(
            #                 Y[day * periods_per_day + p, t] for p in range(periods_per_day) for t in range(limit_workers)  # Para cada motorista t
            ...
            #     for day in range(num_periods // periods_per_week):  # 96 períodos por dia
            ...
            #                 constraint = solver.Add(solver.Sum(Y[i, t] for t in range(limit_workers)) <= 0)  # Impede o trabalho sem descanso após 6 dias consecutivos
            ...
            if (radio_selection_object == "Minimize Total Number of Drivers" and status in (pywraplp.Solver.OPTIMAL, pywraplp.Solver.FEASIBLE)):
                ...
                # # número ótimo de motoristas encontrados na Fase 1
                # best_Z = int(sum(int(Z[t].solution_value()) for t in range(limit_workers)))

                # if best_Z > 0:
                #     # trava o número de motoristas
                #     solver.Add(
                #         solver.Sum(Z[t] for t in range(limit_workers)) == best_Z
                #     )

                #     # troca a função objetivo: agora só balanceamento
                #     solver.Minimize(Lmax)
                ...
                solver.Add(
                    solver.Sum(U[d] for d in range(num_periods)) == best_U
                )
                solver.Add(
                    solver.Sum(Z[t] for t in range(limit_workers)) == best_Z
                )

                # 🎯 novo objetivo: maximizar produção real
                solver.Maximize(
                    solver.Sum(X[d, t] for d in range(num_periods) for t in range(limit_workers))
                )
            ...
            if status in (pywraplp.Solver.OPTIMAL, pywraplp.Solver.FEASIBLE):
                ...
                slot_hours = 0.25
                horas_por_motorista = (
                    matrix_allocation.sum(axis=0) * slot_hours
                ).astype(float).tolist()                        
                

                # total de slots alocados
                # total_workers = sum(int(Z[t].solution_value()) for t in range(limit_workers)) #int(matrix_allocation.sum())            
            ...
            # else:
                ...
                # # -------------------------
                # # trava nº de motoristas
                # # -------------------------
                # solver.Add(solver.Sum(Z[t] for t in range(limit_workers)) <= best_Z)

                # # -------------------------
                # # FASE 2: minimizar Lmax (balancear)
                # # -------------------------
                # solver.Minimize(Lmax)

                # status = solver.Solve()  
                # -----------------------------------------
                # FASE 2 (BALANCEAMENTO): fixa nº motoristas
                # e minimiza Lmax (espalha carga)
                # -----------------------------------------
                # if radio_selection_object == "Minimize Total Number of Drivers" and best_Z > 0:
                #     solver.Add(solver.Sum(Z[t] for t in range(limit_workers)) == best_Z)
                #     solver.Minimize(Lmax)
                ...
                #         # reextrai alocação após rebalanceamento
                #         matrix_allocation = np.array(
                #             [
                #                 [int(Y[d, t].solution_value()) for t in range(limit_workers)]
                #                 for d in range(num_periods)
        ...
        # statistics_result.extend([
        #     f"restrictions: {selected_restrictions}",
        #     f"mode: {radio_selection_object}"
        # ])            
