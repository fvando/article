...
def solve_shift_schedule(solverParamType, need, variable_type, constraints_coefficients, selected_restrictions, swap_rows=None, multiply_row=None, add_multiple_rows=None, densidadeAceitavel=None):
    constraints = []
    msgResult = []
...
with st.form("paramModelo"):
    ...
    with col1:
        total_hours = st.number_input("Quantidade total (horas) | Valores Máximos: | Dia (9h/36per) | Semanal (56h/224per)| Quinzenal (90h/360per):", min_value=1, value=1)
...
if num_periods > 0:
    ...
    st.write(f"Densidade do Modelo: {initialDensityMatrix:.2f}%")
            ...
            with st.expander("Operações", expanded=True):
                ...
                if multiply_row_c:
                    ...
                    if constant != 0: #and st.button("Aplicar Multiplicação"):
                        ...
                        constraints_coefficients = load_data('constraints_coefficients.json')
                        solver, status, total_workers, workers_schedule, constraints_coefficients, initialDensity, finalDensity, statisticsResult, msgResult = solve_shift_schedule(solverParamType, need, variable_type, constraints_coefficients, selected_restrictions, swap_rows=None,multiply_row=multiply_row, add_multiple_rows=None,densidadeAceitavel=percentual_aceitavel)
                        if msgResult is None:
                            save_data(constraints_coefficients, 'constraints_coefficients.json')  # Salva a matriz padrão no arquivo
                    else:
                        st.warning("A constante de multiplicação não pode ser zero!")
                ...
                if add_multiple_rows_c:
                    ...
                    constraints_coefficients = load_data('constraints_coefficients.json')
                    solver, status, total_workers, workers_schedule, constraints_coefficients, initialDensity, finalDensity, statisticsResult, msgResult = solve_shift_schedule(solverParamType, need, variable_type, constraints_coefficients, selected_restrictions, swap_rows=None, multiply_row=None, add_multiple_rows=add_multiple_rows,densidadeAceitavel=percentual_aceitavel)
                    if msgResult is None:
                        save_data(constraints_coefficients, 'constraints_coefficients.json')  # Salva a matriz padrão no arquivo
                else:
                    constraints_coefficients = load_data('constraints_coefficients.json')
