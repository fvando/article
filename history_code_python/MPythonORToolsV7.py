...
def solve_shift_schedule(solverParamType, need, variable_type, constraints_coefficients, selected_restrictions, swap_rows=None, multiply_row=None, add_multiple_rows=None, densidadeAceitavel=None, limitWorkers=0,limit_Iteration=0,limit_Level_Relaxation=0, max_demands_per_driver=1):
    penalty = 1
    constraints = []
    msgResult = []
    # limit_Workers = 10
    ...
    if (radio_selection_Object == "Maximizar Atendimento de Demanda"):
        ...
        solver.Maximize(
            solver.Sum(Y[d, t] * need[d] for d in range(num_periods) for t in range(limit_Workers))
            - penalty * solver.Sum(Y[d, t] for d in range(num_periods) for t in range(limit_Workers))
        )
        # Restrição para distribuir o atendimento de forma proporcional:    
        for d in range(num_periods):
            solver.Add(solver.Sum(Y[d, t] for t in range(limit_Workers)) <= need[d])
    elif (radio_selection_Object == "Minimizar Número Total de Motorista"):
        # Função objetivo: minimizar o número total de trabalhadores
        solver.Minimize(solver.Sum(Y[d, t] for d in range(num_periods) for t in range(limit_Workers)))
        #Adicione uma restrição para limitar o número de slots por motorista
        max_slots_per_driver = 36  # Exemplo: 9 horas por motorista
        for t in range(limit_Workers):
            solver.Add(solver.Sum(Y[d, t] for d in range(num_periods)) <= max_slots_per_driver)
    
    # Restrições de limite diário
    # if max_demands_per_driver != 0:
    #     # solver.Add(solver.Sum([X[d] for d in range(num_periods)]) <= max_slot_Workers)
    #     # solver.Add(solver.Sum(Y[d, t] for t in range(limit_Workers)) <= max_demands_per_driver)
    #     for t in range(limit_Workers):  # Para cada motorista
    #         solver.Add(
    #             solver.Sum(Y[d, t] for d in range(num_periods)) <= max_demands_per_driver
    #         )    
    ...
    if selected_restrictions["cobertura_necessidade"]: 
        for d in range(num_periods):
            constraint_expr = solver.Sum(
                Y[d, t] for t in range(limit_Workers)) >= need[d]  # A demanda deve ser atendida
                
                # X[j] for j in range(num_periods)
                # Y[j, t] for j in range(num_periods) for t in range(limit_Workers)
                
                # if (0 <= (i - j) % num_periods < 18)  # Janela 0 a 18 (condução inicial)
                
                
                # or (19 <= (i - j) % num_periods < 37 and (
                # selected_restrictions.get("divisao_pausa1530", False) or
                # selected_restrictions.get("divisao_pausa3015", False) or
                # selected_restrictions.get("pausa_45_minutos", False))
                # )
    ...
    if limitWorkers != 0:
        constraint = solver.Add(solver.Sum(
            # X[d] for d in range(num_periods)
            Y[d, t] for d in range(num_periods) for t in range(limit_Workers) ) <= limitWorkers)
        constraints.append(constraint)
    ...
    #     # Adicionar pausa de 45 minutos após 4,5 horas de trabalho contínuo
    ...
    #             # Soma dos períodos de pausa nos 3 slots subsequentes
    #             pause = solver.Sum(Y[start + max_continuous_work + p, t] for p in range(pause_duration))
                
    #             # Garantir que a pausa de 45 minutos só ocorre após 18 períodos de trabalho contínuo
    #             constraint_work = solver.Add(continuous_work == max_continuous_work)  # Exatamente 4,5 horas de trabalho
    #             constraints.append(constraint_work)

    #             # Garantir que a pausa de 45 minutos seja aplicada após os 18 períodos
    #             constraint_pause = solver.Add(pause == 0)  # Nenhum trabalho durante a pausa
    #             constraints.append(constraint_pause)
    ...
    if selected_restrictions["limite_diario"]: 
        ...
        for day in range(num_periods // 96):  # 96 períodos de 15 minutos em um dia
            ...
            constraint_expr = solver.Sum(
                ...
                Y[d, t] for d in range(day_start, day_end)) <= max_daily_driving
    ...
    if selected_restrictions.get("divisao_pausa1530", False):
        ...
        for d in range(num_periods):
            constraint_expr = solver.Sum(Y[d, t] for t in range(limit_Workers) 
                # X[j] for j in range(num_periods) 
                if (0 <= (d - t) % num_periods < 18)  # Janela 0 a 18 (condução inicial)
                
                # # Cenario 15/30: Pausa de 15 minutos
                or (19 <= (d - t) % num_periods < 25 and selected_restrictions.get("divisao_pausa1530", False))  # Pausa de 15 minutos
                or (25 <= (d - t) % num_periods < 28 and selected_restrictions.get("divisao_pausa1530", False))
                or (30 <= (d - t) % num_periods < 37 and selected_restrictions.get("divisao_pausa1530", False))
                ...
                # Cenario 45: Pausa de 45 minutos
                or (21 <= (d - t) % num_periods < 37 and selected_restrictions.get("pausa_45_minutos", False))  # Pausa de 45 minutos

                )
            # Adiciona a restrição garantindo que a soma seja maior ou igual à necessidade para o período
            constraint = solver.Add(constraint_expr >= need[d])
            constraints.append(constraint)
    # # Se "divisao_pausa" também estiver marcado, então aplica a lógica de divisão da pausa
    if selected_restrictions.get("divisao_pausa3015", False):
        ...
        for d in range(num_periods):
            constraint_expr = solver.Sum(Y[d, t] for t in range(limit_Workers) 
                ...
                # # Cenario 30/15: Pausa de 30 minutos
                or (20 <= (d - t) % num_periods < 21 and selected_restrictions.get("divisao_pausa3015", False))  # Pausa de 30 minutos
                or (21 <= (d - t) % num_periods < 25 and selected_restrictions.get("divisao_pausa3015", False))
                or (26 <= (d - t) % num_periods < 37 and selected_restrictions.get("divisao_pausa3015", False))
...
with st.form("paramModel"):
    with st.expander("", expanded=True):    
        ...
        with col1:
            ...
            # limit_Workers = st.number_input("Drivers (0, no limit)", min_value=0,  key="limit_workers")
            # max_slot_Workers = st.number_input("Slots (0, no limit)", min_value=1)
        ...
        with col4:
            ...
            limit_Workers = st.number_input("Drivers (0, no limit)", min_value=int(math.ceil(sum(needNew) / int(max_demands_per_driver))),  key="limit_workers")
