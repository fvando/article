...
import time  # Importa o módulo para manipular tempo
...
@st.cache_resource
def gerar_valores_aleatorios_cache(total_hours, period_minutes):
...
def save_data(data, FILENAME):
    ...
    # # Se houver dados existentes, fazer o merge com os novos dados
    # if existing_data.size > 0:
    #     # Verificar se os formatos são diferentes
    #     if existing_data.shape != data.shape:
    #         # Ajustar o tamanho das matrizes para serem compatíveis
    #         max_shape = np.maximum(existing_data.shape, data.shape)
    #         # Expandir as matrizes para o tamanho máximo
    #         existing_data_resized = np.zeros(max_shape, dtype=int)
    #         data_resized = np.zeros(max_shape, dtype=int)
            
    #         existing_data_resized[:existing_data.shape[0], :existing_data.shape[1]] = existing_data
    #         data_resized[:data.shape[0], :data.shape[1]] = data

    #         # Realizar o merge (OR) com os dados existentes
    #         data = np.maximum(data_resized, existing_data_resized)
    
    # # Salvar a matriz atualizada no arquivo
    # with open(FILENAME, 'w') as f:
    #     json.dump(data.tolist(), f)  # Converte a matriz NumPy para lista antes de salvar
...
def solve_shift_schedule(solverParamType, need, variable_type, constraints_coefficients, selected_restrictions, swap_rows=None, multiply_row=None, add_multiple_rows=None, densidadeAceitavel=None, limitWorkers=0):
    ...
    #rowsC, colsC = constraints_coefficients.shape
    # Verifique se o número total de elementos é o mesmo
    #if (rowsC * colsC) != (num_periods * num_periods):
    #    print("Erro: O número total de elementos não pode ser compatível com o novo tamanho.")
    #else:
    # Redimensionando a matriz (se possível)
    #constraints_coefficients = constraints_coefficients.reshape((num_periods, num_periods))


    #if (int(num_periods) != rowsC) and (int(num_periods) != colsC):
        # Redimensionando a matriz para 3x3
        #constraints_coefficients = np.resize(constraints_coefficients, (num_periods, num_periods))
        #constraints_coefficients = constraints_coefficients.reshape((num_periods, num_periods))
    ...
    if selected_restrictions["cobertura_necessidade"]: 
        for i in range(num_periods):
            constraint = solver.Add(solver.Sum(X[j] for j in range(num_periods)) >= int(need[i]))
            constraints.append(constraint)
        
        # Verificando o número de linhas e colunas
        # rowsC, colsC = constraints_coefficients.shape
        # if (num_periods != rowsC) and (num_periods != colsC):
        #     # Redimensionando a matriz para 3x3
        #     constraints_coefficients = np.resize(constraints_coefficients, (num_periods, num_periods))
    
        for ii in range(num_periods):
            for jj in range(num_periods):
                # Se há cobertura, marcamos como 1
                constraints_coefficients[ii, jj] = 1  # Sempre marca como cobertura
            
        # Depuração: Imprimir a matriz
        #print("Matriz constraints_coefficients:")
        #for row in constraints_coefficients:
        #    print(row)
    ...
    if finalDensity <= densidadeAceitavel:
        ...
        while status != pywraplp.Solver.OPTIMAL and iteration < max_iterations:
            statisticsResult.append(f"Iteração {iteration + 1}: Conflito detectado. Relaxando restrições...")
        ...
        if status == pywraplp.Solver.OPTIMAL:
            ...
            # for i in range(num_periods):
            #     statisticsResult.append(f"X[{i}] = {X[i].solution_value()}")
        ...
        #elif status == pywraplp.Solver.INFEASIBLEFEASIBLE:
        #    statisticsResult.append(f"Estado do Modelo: Inviável não há solução viável com as restrições fornecidas")
        elif status == pywraplp.Solver.UNBOUNDED:
            statisticsResult.append(f"Estado do Modelo: Sem limites: verifique se as variáveis têm restrições adequadas.")
        else:
...
with st.form("paramModelo"):
    ...
    with col1:
        ...
        percentual_aceitavel = st.number_input("Densidade Aceitável p/ Execução do Modelo:", min_value=0, value=1)
        ...
        if atualizarFicheiro:
            ...
            if constraints_coefficients.size > 0:
                save_data(constraints_coefficients, 'constraints_coefficients.json')                
            if initualconstraintsCoefficients.size > 0:
    ...
    with col2:
        ...
        # Condicional para definir `default_need` com valores fixos ou novos aleatórios
        if fixar_valores:
            default_need = gerar_valores_aleatorios_cache(total_hours, period_minutes)
        else:
...
if submit_button:
    ...
    # atualizar a Matriz de Restrição
    if atualizarFicheiro:
      initualconstraintsCoefficients = np.ones((num_periods, num_periods), dtype=int)         
      save_data(initualconstraintsCoefficients, 'initualconstraintsCoefficients.json')
      constraints_coefficients = initualconstraintsCoefficients 
...
#if initualconstraintsCoefficients is None or initualconstraintsCoefficients.size == 0:
#    initualconstraintsCoefficients = np.ones((num_periods, num_periods), dtype=int)
#constraints_coefficients = np.ones((num_periods, num_periods), dtype=int)
#for i in range(num_periods):
...
#         if (0 <= (i - j) % num_periods <= 36):
#        initualconstraintsCoefficients[i, j] = 1  # Atualiza com cobertura
#         else:
#            initualconstraintsCoefficients[i, j] = 0  # Sem cobertura
...
if num_periods > 0: #inicio do codigo, não preencheu a matriz
    ...
    if len(need) != num_periods:
        ...
        with col_resultsI:
            ...
            with st.expander("Operações", expanded=True):
                ...
                add_multiple_rows_c_auto = st.checkbox("Somar Múltiplo de uma Equação a Outra - Automatico")
                ...
                if (not swap_rows_c 
                    ...
                    if add_multiple_rows_c_auto:
                        ...
                        # Exibe cada linha da matriz com um loop
                        for idx, row in enumerate(constraints_coefficients):
                            ...
                            if finalDensity <= percentual_aceitavel:
                                st.write(f"Final Density {finalDensity} atingiu o limite aceitável ({percentual_aceitavel}). Saindo do loop.")
                                break
        ...
        if msgResult is None:    
            if initialDensityMatrix is not None:
                ...
                with st.expander("Matriz", expanded=True):
                    ...
                    sns.heatmap(initualconstraintsCoefficients, cmap="Blues", cbar=False, annot=False, fmt="d", annot_kws={"size": 8})
                    #sns.heatmap(initualconstraintsCoefficients, cmap="binary", cbar=False,linewidths=0.09) #, ax=ax, linewidths=0.01, annot=False, fmt="d")
    ...
    with col_resultsIniII:
        if msgResult is None:
            ...
            with st.expander("Matriz", expanded=True):
                ...
                sns.heatmap(initualconstraintsCoefficients, cmap="gray_r", cbar=False, annot=False, fmt="d", annot_kws={"size": 8})
                #sns.heatmap(initualconstraintsCoefficients, cmap="gray_r", cbar=False) #, ax=axNew, linewidths=0.01, annot=False, fmt="d")
    ...
    with col_resultsIniIII:
        if msgResult is None:
            ...
            with st.expander("Matriz", expanded=True):
                figNew, axNew = plt.subplots(figsize=(14, 8))
                constraints_coefficients = load_data('constraints_coefficients.json')
                sns.heatmap(constraints_coefficients, cmap="Oranges", cbar=False, annot=False, fmt="d", annot_kws={"size": 8})
                #sns.heatmap(constraints_coefficients, cmap="gray_r", cbar=False) #, ax=axNew, linewidths=0.01, annot=False, fmt="d")
                plt.title('Matriz de Coeficientes')
                plt.xlabel('X')
                plt.ylabel('Períodos')
                st.pyplot(figNew)
